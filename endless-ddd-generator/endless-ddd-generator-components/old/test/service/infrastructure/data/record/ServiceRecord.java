...
import org.endless.tianyan.metadata.components.generator.service.domain.type.*;
...
public class ServiceRecord implements DDDGeneratorRecord<ServiceAggregate> {
    ...
    private void validateVersion() {
        if (!StringUtils.hasText(version)) {
            throw new DataRecordValidateException("服务版本号不能为空");
        }
    }
    ...
    private void validateModifyAt() {
        if (modifyAt == null || modifyAt < 0) {
            throw new DataRecordValidateException("服务更新时间不能为 null 或小于 0，当前值为: " + modifyAt);
        }
    }
