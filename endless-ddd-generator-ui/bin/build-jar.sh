...
if ! cp "../endless-ddd-generator/endless-ddd-generator-core/target/endless-ddd-generator.jar" "lib/"; then
...
if ! cp "../endless-ddd-generator/endless-ddd-generator-core/config/application.yaml" "config/" 2>/dev/null; then
    echo "Warning: application.yaml copy failed, using default"
fi

if ! cp "../endless-ddd-generator/endless-ddd-generator-core/config/log4j2-prod.xml" "config/" 2>/dev/null; then
    echo "Warning: log4j2-prod.xml copy failed"
fi

if ! cp "../endless-ddd-generator/endless-ddd-generator-core/config/application-prod.yaml" "config/" 2>/dev/null; then
