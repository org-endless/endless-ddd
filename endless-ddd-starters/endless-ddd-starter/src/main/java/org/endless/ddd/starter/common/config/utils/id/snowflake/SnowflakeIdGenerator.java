package org.endless.ddd.starter.common.config.utils.id.snowflake;

import org.endless.ddd.starter.common.exception.config.utils.id.SnowflakeIdGeneratorException;
...
public class SnowflakeIdGenerator {
    ...
    public Long nextId() {
        lock.lock(); // 加锁
        try {
            Long timestamp = TimestampTools.now();

            if (timestamp < lastTimestamp) {
                if (lastTimestamp - timestamp < TIME_WINDOW) {
                    timestamp = lastTimestamp;
                } else {
                    throw new SnowflakeIdGeneratorException("服务器时间回拨，请检查系统时间是否正确，上次请求UTC时间戳: " + lastTimestamp);
                }
            }
    ...
    private void validateIds(Long dataCenterId, Long workerId) {
        if (dataCenterId == null || workerId == null) {
            throw new SnowflakeIdGeneratorException("数据中心ID和服务器ID不能为空");
        }
        if (dataCenterId < 0 || dataCenterId > MAX_DATA_CENTER_ID) {
            throw new SnowflakeIdGeneratorException("数据中心ID无效或超出 0-15 范围: " + dataCenterId);
        }
        if (workerId < 0 || workerId > MAX_WORKER_ID) {
            throw new SnowflakeIdGeneratorException("服务器ID无效或超出 0-63 范围: " + workerId);
        }
    }

    private Long waitNextMillis(Long lastTimestamp) {
        Optional.ofNullable(lastTimestamp)
                .orElseThrow(() -> new SnowflakeIdGeneratorException("lastTimestamp 不能为空"));
        Long timestamp = TimestampTools.now();
        while (timestamp.compareTo(lastTimestamp) <= 0) {
            timestamp = TimestampTools.now();
        }
        return timestamp;
    }
}
